...
public class LsShellCommand implements ShellCommand {
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		try {
			directoryPath = Utility.resolveDir(Paths.get(arguments.isBlank() ? "" : args[0]), env);
	...
	/**
	 * Writes the given directory listing.
	 * 
	 * @param directoryPath path of the parent directory.f
	 * @param env           environment program works in.
	 * @throws IOException If there is a problem with the given path.
	 */
	private void printData(Path directoryPath, Environment env) throws IOException {
